  ...
  if (unlimitedPattern) {
    ...
    unlimitedPattern.trim().split(/\s+/).forEach(function(unlimitedHost, i) {
      ...
      if (startsWithSlash || endsWithSlash) {
        ...
        try {
          // Validate the regex pattern by attempting to construct a RegExp object.
          new RegExp(unlimitedHost);
        } catch (e) {
          throw new Error('Invalid regex pattern at index ' + i + ': ' + unlimitedHost);
        }
        // Escape the regex pattern to prevent injection.
        unlimitedHost = _.escapeRegExp(unlimitedHost);
      } else {
