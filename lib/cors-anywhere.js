...
function parseURL(req_url) {
  ...
  // Validate hostname and protocol against allowlist
  const allowedHostnames = ['example.com', 'api.example.com']; // Add allowed hostnames here
  const isAllowedHostname = allowedHostnames.some((allowed) => 
    parsed.hostname === allowed || parsed.hostname.endsWith(`.${allowed}`)
  );
  if (!isAllowedHostname || (parsed.protocol !== 'http:' && parsed.protocol !== 'https:')) {
    return null; // Reject URLs with disallowed hostnames or protocols
  }
  return parsed;
}
